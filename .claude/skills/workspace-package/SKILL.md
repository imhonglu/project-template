...
설명·예시 요청에는 구성안을 제공합니다. 생성 요청에는 기존 패키지 이름과 공통 설정을 확인해 아래 구성을 적용합니다.
...
의존성이 없어도 설치를 실행해 새 패키지를 잠금 파일에 반영합니다.
...
패키지를 생성했다면 파일을 정리하고 루트 전체 검사를 실행합니다. 새 패키지의 타입 검사도 포함됐는지 확인합니다.
