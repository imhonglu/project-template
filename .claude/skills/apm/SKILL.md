...
후보의 원본에서 실제 지원 기능·실행 조건·출처·유지보수 상태를 확인합니다. 검색 요약이나 인기도만으로 추천하지 않습니다. 결과가 없거나 조회에 실패하면 검색어·출처를 바꾸고 공식 문서와 교차 확인합니다.

#### 스킬

[skills.sh](https://skills.sh/)와 Skills CLI에서 후보를 찾고 원본 `SKILL.md`와 필요한 참고 자료를 읽습니다. 스킬이 사용하는 MCP 등 연결 도구도 확인합니다.
...
`apm.yml`의 의존성·배포 대상과 설치 상태를 확인합니다.
...
`includes: auto`를 사용하는 프로젝트에서 `.apm/skills/<name>/SKILL.md`를 원본으로 작성합니다. 이름은 소문자와 하이픈을 사용하고 폴더명과 맞춥니다. 아래 뼈대에 작업별로 필요한 절을 더합니다.
...
- `description`에는 수행할 작업과 선택할 맥락을 적습니다. `사용 시점`에는 상황별 대표 요청을 두고 비슷한 예시는 줄입니다.
- 순서가 중요한 작업은 번호 목록, 여러 작업을 지원하면 소제목으로 구성합니다. 메시지 형식·충돌 해결 등 추가 절은 필요할 때만 둡니다.
- 명령·설정 예시는 선택을 명확히 하거나 그대로 사용할 가치가 있을 때 남깁니다. 기본 사용법·반복 설명을 줄이되 필요한 제약을 생략하지 않습니다.
- `결과 확인`에는 완료 판단과 보고할 내용만 둡니다. 관련 파일이 있다는 이유로 무관한 작업에 적용하지 않도록 범위를 정합니다.

배포본에만 있는 수정도 비교·반영하고, 원본을 정리한 뒤 배포본을 재생성합니다. `apm.yml`을 수정한 경우에도 같은 명령을 사용합니다.
...
서버 제공 방식에 맞는 명령으로 `apm.yml`의 `dependencies.mcp`에 서버를 추가하고 대상별 설정을 배포합니다.
...
Codex의 프로젝트 MCP 설정을 배포하려면 `.codex/` 디렉터리가 있어야 합니다.
...
- 설치·변경 후 `apm.yml`·`apm.lock.yaml`과 대상별 배포 결과를 확인합니다. 잠금 파일은 APM으로 갱신하고, 대상별 지원 차이와 생략 이유를 보고합니다.
- 로컬 스킬은 `.agents/skills/`·`.claude/skills/`의 배포본과 원본을 비교합니다. MCP는 `.codex/config.toml`·`.mcp.json`과 대상 도구의 연결 상태를 확인합니다.
- 이름 변경·제거 후 남은 생성물은 사용자 수정과 소유 관계를 확인해 정리합니다.
