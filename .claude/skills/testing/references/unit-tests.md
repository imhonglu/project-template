...
주문 금액에 따른 배송비의 정상·경계·실패 사례를 검증합니다.
...
- 기대값은 고정값으로 작성합니다. 테스트에서 배송비 계산식을 다시 구현하지 않습니다.
- 잘못된 입력도 함수가 받는 타입 안에서 선택합니다. 위 실패 사례는 모두 `number`이며, `as any`로 타입 검사를 우회할 필요가 없습니다.
