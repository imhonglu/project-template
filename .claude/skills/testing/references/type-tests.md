...
일반 `.test.ts` 파일에서 함께 작성하되, 값은 Vitest 실행으로, `expectTypeOf`는 테스트 파일을 포함한 프로젝트의 `typecheck`로 검증합니다.
...
기대 타입은 호출자의 계약을 직접 명시합니다. 위 예제는 반환 타입을 `unknown`으로 넓히면 `typecheck`에서 실패합니다. `ReturnType<typeof firstItem>`에서 기대 타입을 가져오면 잘못된 변경도 따라가므로 회귀를 잡을 수 없습니다.
