...
[skills.sh](https://skills.sh/)와 Skills CLI에서 후보를 찾습니다. 원본 `SKILL.md`뿐 아니라 해당 작업에서 연결하는 참고 파일·예제도 읽고 적용 조건을 확인합니다.
...
- 스킬 구조를 정할 때는 공식 작성 가이드와 유사한 공개 스킬의 본문·참고 파일을 비교해 적용할 패턴과 이유를 정합니다.
- 에이전트가 기본 도구 사용법을 안다고 전제합니다. 짧은 지침과 완성된 대표 예제로 설명하고, 예제에서 드러나지 않는 판단·제약을 덧붙입니다.
- `description`에는 수행할 작업과 선택할 맥락·제외 범위를 적습니다. `사용 시점`에는 상황별 대표 요청을 두고 비슷한 예시는 줄입니다.
- 순서가 중요한 작업은 번호 목록, 여러 작업을 지원하면 소제목으로 구성합니다. 짧은 기본 예제와 놓치면 안 되는 제약은 해당 작업 절에 둡니다.
- 긴 예제·특정 환경의 절차는 참고 파일로 분리합니다. 필요한 작업 절에서 읽을 조건과 상대 링크로 직접 연결하고, 선택지가 많을 때만 표로 묶습니다.
- 참고 파일은 적용 조건 → 예제 → 변형·주의점 순서로 설명합니다. 예제에 필요한 구현·설정·파일을 밝히고, 같은 예제의 파일명·API 이름을 일관되게 사용합니다. 중복 설명·불필요한 절을 줄이고 공식 근거는 해당 내용 옆에 연결합니다.
- `결과 확인`에는 완료 판단·검증·보고할 내용만 둡니다. 기본 사용법·반복 설명을 줄이되 필요한 제약은 유지합니다.

다음은 README 작성 예제를 필요한 작업에 연결하는 구성입니다.

```markdown
---
name: package-readme
description: 패키지 README에 소개·사용법·개발 방법을 작성합니다. README 생성·정비 요청에 사용합니다.
---
...
- 문서 정리: "하위 패키지 README에 사용법과 개발 방법을 정리해줘"

## 작업 절차

소개 → 사용법 → 개발 순서로 작성합니다.
README를 생성·정비할 때 [README 예제](references/package-readme.md)를 읽고,
현재 API와 실행 명령에 맞춥니다.

## 결과 확인

예제의 공개 경로·명령·실행 조건을 실제 코드·설정과 대조합니다.
```

참고: [Agent Skills 작성 기준](https://agentskills.io/skill-creation/best-practices)·[Anthropic의 본문 예제와 작업별 문서 연결](https://github.com/anthropics/skills/blob/main/skills/pdf/SKILL.md)·[Vercel의 규칙별 코드 예제](https://github.com/vercel-labs/agent-skills/blob/main/skills/react-best-practices/rules/async-parallel.md).
...
- 스킬 구조를 바꿨다면 대표 요청에서 필요한 참고 파일을 찾고 적용할 수 있는지 확인합니다. 링크·예제의 선행 파일·실행 조건을 대조하고, 변경한 명령 옵션은 도구 도움말로, 코드 예제는 해당 환경의 검사·실행으로 검증합니다.
