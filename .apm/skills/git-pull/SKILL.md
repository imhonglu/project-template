...
1. 현재 브랜치·원격·병합 대상과 진행 중인 Git 작업을 확인합니다.
   - 이미 병합 중이면 해당 충돌 해결부터 이어갑니다.
   - 미커밋 변경은 임의로 커밋하지 않고, 필요 시 stash에 보관해 생성한 항목을 기록합니다.
   - 병합을 막는 미추적 파일도 보관하며, 기존 스테이징 상태를 보존합니다.
...
3. 병합할 원격을 fetch합니다. 기본 명령은 `git fetch origin`입니다.
   - 원격 작업 브랜치의 존재·추적 관계를 확인합니다.
   - 원격 작업 브랜치가 없거나 기준 브랜치와 같으면 생략합니다.
   - 가능한 경우 fast-forward하고, 분기한 이력은 확인 후 merge합니다.
4. 원격 작업 브랜치 → 기준 브랜치 순서로 병합합니다. 각 병합을 검증·완료한 뒤 다음으로 진행합니다.

   아래 명령은 병합 커밋 전에 멈춥니다. fast-forward는 즉시 완료되므로 직후 검증합니다.
...
5. 충돌을 해결하고 각 병합을 검증·완료합니다.
   ...
   - 저장소 검사를 실행하고 이번 병합으로 생긴 실패를 해결합니다.
   - 메시지 규칙과 훅을 적용해 병합을 커밋합니다. fast-forward이면 추가 커밋은 만들지 않습니다.
   - 해결할 수 없으면 후속 병합을 중단하고 상태·원인을 보고합니다. 이번 작업에서 시작한 진행 중 병합은 `git merge --abort`로 취소할 수 있습니다. 완료된 fast-forward에는 적용하지 않습니다.
6. 생성한 stash를 지정해 `git stash apply --index <stash>`로 사용자 변경을 복원합니다.
   - 충돌이 있으면 stash를 유지한 채 해결합니다.
   - 변경 내용·스테이징 상태를 확인한 뒤 이번 작업에서 만든 항목만 제거합니다.
